def deep_map(f, s):
    # 编写一个函数 deep_map，它接受一个列表 s 和一个单参数函数 f。
    # s 可能是一个嵌套列表，即包含其他列表的列表。
...
def balanced(m):
    ...
    if is_planet(m):
        ...
        p_l,p_r=end(left(m)),end(right(m))
        return total_mass(p_l)*length(left(m))==total_mass(p_r)*length(right(m)) and balanced(p_l) and balanced(p_r)
