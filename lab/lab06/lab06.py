...
def make_change(amount, coins):
    # 实现 make_change 函数，该函数接受一个正整数 amount 和一个硬币字典 coins。
    # coins 字典的键是正整数面额，值是正整数硬币数量。
    # 例如，{1: 4, 5: 2} 表示有四个1分的硬币和两个5分的硬币。
    # make_change 函数返回一个硬币列表，其总和为 amount，其中任意面额 k 的硬币使用数量不超过 coins[k]。
