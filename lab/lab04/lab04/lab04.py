...
def buy(required_fruits, prices, total_amount):
    ...
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:
            ...
            fruit = fruits[0]
            price = prices[fruit]
            sum_remain_min=sum(prices[i] for i in fruits[1:])
            max_k=(amount-sum_remain_min)//price
...
def closer_city(lat, lon, city_a, city_b):
    ...
    compare1=abs(distance(city_a,city_c))
    compare2=abs(distance(city_c,city_b))
    return get_name(city_a) if compare1<compare2 else get_name(city_b)
