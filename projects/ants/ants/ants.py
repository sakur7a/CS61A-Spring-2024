...
class ThrowerAnt(Ant):
    ...
    food_cost = 3
    initial_health=1
    lower_bound = 0  # 攻击距离下限
    upper_bound = float('inf')  # 攻击距离上限
    ...
    def nearest_bee(self):
        ...
        while current_place is not None:
            if not current_place.is_hive and current_place.bees and self.lower_bound<=step<=self.upper_bound:
                return random_bee(current_place.bees)
            current_place = current_place.entrance  # 向前检查下一个位置
            step+=1
        return None  # 没有找到符合条件的蜜蜂
        # END Problem 3 and 4
...
class QueenAnt(ThrowerAnt):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 12
        super().reduce_health(amount)  # 先调用父类方法减少健康值
        if self.health <= 0:
            ants_lose()  # 触发游戏结束
        # END Problem 12
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not start_place.is_hive:
            if start_place.ant:
                self.dist[start_place.ant]=len
            if start_place.ant and start_place.ant.is_container and start_place.ant.ant_conatined:
                self.dist[self.place.ant.ant_contained]=len
            if start_place.bees:
                for bee in start_place.bees:
                    self.dist[bee]=len
            len+=1
